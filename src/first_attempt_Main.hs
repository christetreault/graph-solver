...
main = do
   ...
   where
      testData w d = runEval $ evalList rdeepseq $ testGraph w d (mkStdGen 800815) nFn_sleep
...
sFn_sleep :: Int -> [Float] -> IO Float
sFn_sleep n s = do
   let pOMF = (0.01 :: Float) * fromIntegral n
   _ <- system $ "sleep " ++ (show pOMF)
   return $ foldl' (+) pOMF s
